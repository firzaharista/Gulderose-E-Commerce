...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', nama,username,id_group,email, id, password, active, last_login')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
	...
	public function login_front($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, nama, username, password, phone, address,id_kota,id_provinsi, id_group, active, last_login')
			->where($this->identity_column, $identity)
			->limit(1)
			->order_by('id', 'desc')
			->get($this->tables['users']);
	...
	{
		$this->db->join('users_group', 'users.id_group = users_group.id_group');
		return $this->db->get($this->table);
	}

	function get_users_superadmin()
	{
		$this->db->join('users_group', 'users.id_group = users_group.id_group');
		$this->db->order_by('users.id_group');
		$this->db->where('users.id_group', 1);
		return $this->db->get($this->table);
	}
	...
	function get_users_admin()
	{
		$this->db->join('users_group', 'users.id_group = users_group.id_group');
		$this->db->order_by('users.id_group');
		$this->db->where('users.id_group', 2);
		return $this->db->get($this->table);
	}

	//ambil user buyer/pembeli
	function get_users_buyer()
	{
		$this->db->join('users_group', 'users.id_group = users_group.id_group');
		$this->db->order_by('users.id_group');
		$this->db->where('users.id_group', 3);
		return $this->db->get($this->table);
	}

	// tambahan lain (mengambil data users group)
	...
	function get_all_users_group2()
	{
		$query = $this->db->get('users_group');
		if ($query->num_rows() > 0) {
			foreach ($query->result_array() as $row) {
				$result[''] = '- Pilihan Level User -';
				$result[$row['id_group']] = ucwords(strtolower($row['nama_group']));
			}
			return $result;
		}
	}
	...
	// 		$data = array();
	// 		foreach ($query->result_array() as $row) {
	// 			$data[$row['id_group']] = $row['nama_group'];
	// 		}
	// 		return $data;
	// 	}
	// }
	public function profil()
	{
		...
		$this->db->join('users_group', 'users.id_group = users_group.id_group', 'left');
	...
	{
		$bar = array('id' => $id);
		return $this->db->get_where('users', $bar);
	}
